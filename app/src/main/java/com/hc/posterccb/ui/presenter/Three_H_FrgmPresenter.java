...
import com.hc.posterccb.ui.model.Tree_H_Model;
...
public class Three_H_FrgmPresenter extends BasePresenter<Second_H_Fragment> implements Second_H_Constract.FrgmPresenter {
    @Override
    public void getProgramList(Program program) {
        if (program == null) return;
        if (!getIView().checkPostParamNull()) {
            ((Tree_H_Model) getiModelMap().get("postplaylog")).getProgram(program, new Tree_H_Model.InfoHint() {
                @Override
    ...
    public HashMap<String, IModel> getiModelMap() {
        return loadModelMap(new Tree_H_Model());
    }
